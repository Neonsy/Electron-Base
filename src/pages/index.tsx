...
const coreStack: FeatureCard[] = [
    ...
    },
    {
        ...
        desc: 'Mutative middleware for ergonomic immutable Zustand updates',
        icon: VscFlame,
    ...
    {
        name: 'evlog',
        desc: 'Structured app and IPC request logging for Electron',
        icon: VscTerminal,
        gradient: 'from-sky-400 to-cyan-500',
    },
    {
        name: 'neverthrow',
        desc: 'Explicit Result-based async error handling',
        icon: VscShield,
...
const keyFiles = [
    { path: 'electron/main/index.ts', desc: 'Main process entry', color: 'text-electric-400' },
...
const runtimeExample = `import { log } from 'evlog';
import { ResultAsync } from 'neverthrow';

void ResultAsync.fromPromise(
  trpcClient.system.signalReady.mutate(),
    ...
    return (
        <div className='bg-obsidian-950 relative min-h-screen overflow-hidden text-white'>
            ...
            <div className='relative z-10 mx-auto max-w-7xl px-6 py-16 lg:py-24'>
                ...
                <section className='animate-slide-up mb-32 delay-600'>
                    <div className='mb-10 flex items-center gap-4'>
                        ...
                        <div>
                            ...
                            <p className='text-white/40'>Zustand + zustand-mutative middleware</p>
                        </div>
                    </div>
                ...
                {/* IPC + Observability */}
                <section className='animate-slide-up mb-32 delay-650'>
                    <div className='mb-10 flex items-center gap-4'>
                        <div className='from-electric-500 flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br to-cyan-500'>
                            <VscPlug aria-hidden='true' focusable='false' className='h-6 w-6 text-white' />
                        </div>
                        <div>
                            <h2 className='text-3xl font-bold text-white'>IPC, Logs, and Results</h2>
                            <p className='text-white/40'>tRPC IPC with minimal evlog + neverthrow glue</p>
                        </div>
                    </div>
                    ...
                    <div className='glass-card overflow-hidden rounded-2xl'>
                        <div className='flex items-center justify-between border-b border-white/10 bg-white/5 px-6 py-4'>
                            ...
                            <span className='font-mono text-xs text-white/30'>boot.ts</span>
                        </div>
                        <pre className='overflow-x-auto p-6 font-mono text-sm leading-relaxed'>
                            <code className='text-white/70'>
                                {runtimeExample.split('\n').map((line, i) => (
                                    <div key={i} className='flex'>
                                        <span className='mr-6 w-6 text-right text-white/20 select-none'>{i + 1}</span>
                                        <span>{line}</span>
                                    </div>

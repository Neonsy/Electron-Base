...
- **Mutative + zustand-mutative**: Fast immutable updates with a dedicated Zustand middleware
...
- **evlog**: Structured logging for Electron main process and tRPC IPC request flow
- **neverthrow**: Result-based async error handling for renderer and backend workflows
...
│   │   ├── bootstrap/       # App lifecycle orchestration
│   │   ├── window/          # Window creation + navigation guards
│   │   ├── security/        # CSP + URL policy modules
│   │   ├── updates/         # Auto-update logic
│   │   ├── preload/         # Preload script (tRPC bridge)
...
The template includes an auto-updater setup in `electron/main/updates/updater.ts` that:
...
```ts
// electron/main/updates/updater.ts (versions 0.1.0-alpha.1 through 0.1.0-alpha.5)
...
```ts
// electron/main/updates/updater.ts (version 0.1.0-alpha.6, the "migration" release)
...
```ts
// electron/main/updates/updater.ts (versions 0.1.0-beta.1 onwards)
...
```ts
// electron/main/updates/updater.ts (version 0.1.0-beta.5, the "migration" release)
...
```ts
// electron/main/updates/updater.ts (versions 1.0.0 onwards)
...
- `electron/main/window/factory.ts`: Creates hidden window, waits for tRPC signal
...
CSP is managed centrally in `electron/main/security/cspPolicy.ts` and enforced via HTTP headers in `electron/main/security/cspHeaders.ts`. The header is the single source of truth.

**Where CSP is defined:**

- `electron/main/security/cspPolicy.ts`: Central CSP configuration module (source of truth)
- `electron/main/security/cspHeaders.ts`: Injects CSP headers via `session.webRequest.onHeadersReceived`
...
To allow an external API (e.g., `https://api.example.com`), add it to `connect-src` in `electron/main/security/cspPolicy.ts`:
...
Check which directive is blocking the resource and add the appropriate origin to `electron/main/security/cspPolicy.ts`.

**Using Convex with CSP:**

If you add [Convex](https://convex.dev/) as a backend, update `electron/main/security/cspPolicy.ts` to allow Convex's domains:
...
This template includes [Zustand](https://zustand.docs.pmnd.rs/) for state management with [Mutative](https://mutative.js.org/) and the dedicated [zustand-mutative](https://www.npmjs.com/package/zustand-mutative) middleware package.
...
- Uses the dedicated Zustand integration package instead of plain `mutative` alone
- Keeps the template docs and installed dependencies aligned
- Preserves the familiar "mutate a draft" authoring style with better performance than Immer
...
### Observability and Error Handling: evlog + neverthrow

The template includes [evlog](https://www.npmjs.com/package/evlog) for structured logging and [neverthrow](https://github.com/supermacro/neverthrow) for explicit `Result`/`ResultAsync` flows.

The default setup now uses them in two lightweight places:

- Renderer boot uses `ResultAsync` to wrap the initial `system.signalReady` IPC call
- Main-process tRPC middleware emits an `evlog` request log for each IPC procedure when logging is enabled

**Renderer boot example:**

```ts
import { log } from 'evlog';
import { ResultAsync } from 'neverthrow';

void ResultAsync.fromPromise(trpcClient.system.signalReady.mutate(), (error) => new Error(String(error))).match(
    () => log.info('window', 'Renderer ready signal acknowledged.'),
    (error) => log.warn('window', error.message)
);
```

Enable pretty logs in development with:

```bash
EVLOG_ENABLED=1 pnpm dev
```
